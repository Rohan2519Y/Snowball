...
export default function CompanyProductManagement() {
    ...
    // Calculate summary
    const calculateSummary = (productData) => {
        let totalOrdered = 0;
    ...
    // Handle Delete
    const handleDelete = async (companyproductid) => {
    ...
    const handleEntryChange = (index, field, value) => {
        ...
        if (field === 'orderedqty' || field === 'icecreamname') {
            ...
            if (iceCream) {
                ...
                newEntries[index].orderedamount = (price * qty).toFixed(2);
            }
        }
        ...
        if (field === 'deliveredqty' || field === 'icecreamname') {
            ...
            if (iceCream) {
                ...
                newEntries[index].deliveredamount = (price * qty).toFixed(2);
            }
        }
        ...
        try {
            ...
            if (hasMultipleEntries && isAddMode) {
                ...
                if (allSuccess && successCount > 0) {
                    alert(`${successCount} products added successfully!`);
                    setIsModalOpen(false);
                    fetchProducts();
                } else if (successCount > 0) {
                    alert(`${successCount} products added successfully. ${failedCount} products failed.`);
                    setIsModalOpen(false);
                    fetchProducts();
                } else {
                    alert('Failed to add products. Please check your entries.');
                }
            } else {
                // Save single entry
                let result;
    ...
    const getStockStatus = (remaining) => {
        if (remaining <= 0) {
            ...
            return { label: 'Medium Stock', className: 'bg-yellow-100 text-yellow-700' };
        } else {
                                    ...
                                    <div key={index} className="grid grid-cols-1 md:grid-cols-6 gap-3 items-center border-b border-gray-100 pb-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                className="px-2 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm"
                                            >
                            ...
                            {products.map((product, index) => {
                                ...
                                return (
                                    <tr key={product.companyproductid} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-blue-600">₹{parseFloat(product.orderedamount || 0).toFixed(2)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-semibold">{product.deliveredqty}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-green-600">₹{parseFloat(product.deliveredamount || 0).toFixed(2)}</td>
